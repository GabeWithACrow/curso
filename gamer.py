...
app = FastAPI()
database = '/Users/Ijovem01/thor/database/game.db'

@app.get("/listar")
def listar():
    c = sqlite3.connect(database)
    cr = c.cursor()
    cr.execute("""select nome, apelido, email, nascimento, status from jogador""")
    lista=cr.fetchall()
    c.close()
    return (lista)
...
def inserir(player: Jogador):
    c = sqlite3.connect(database)
    cr = c.cursor()
    cr.execute("""  insert into jogador (nome, apelido, email, nascimento, status) values (?, ?, ?, ?, ?)  """, (player.nome, player.apelido, player.email, player.nascimento, player.status))
    c.commit()
    c.close()
    return 0
